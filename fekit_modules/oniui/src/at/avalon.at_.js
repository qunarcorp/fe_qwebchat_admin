...
;
                    ...
                    vm._popup = function (str) {
                        ...
                        if (document.createRange && document.documentMode != 9) {
                            //如果是IE10+或W3C
    ...
    widget.defaults = {
        ...
        //默认的标识符,
        datalist: [],
        //字符串数组，不可监控，(名字取自HTML的datalist同名元素)
        _datalist: [],
        //实际是应用于模板上的字符串数组，它里面的字符可能做了高亮处理
        template: '',
        //弹出层的模板，如果为空，使用默认模板，注意要在上面添加点击或hover处理
        toggle: false,
        //用于控制弹出层的显示隐藏
        activeIndex: 0,
        //弹出层里面要高亮的列表项的索引值
        ...
        //弹出层里面总共有多少个列表项
        maxLength: 20,
        //@后的查询字符串的最大长度，注意中间不能有空格
        minLength: 1,
        //@后的查询字符串只有出现了多少个字符后才显示弹出层
        delay: 500,
        //我们是通过$update方法与后台进行AJAX连接，为了防止输入过快导致频繁，需要指定延时毫秒数
