    ...
    */
    ...
    var mmState = window.mmState = {
            ...
            transitionTo: function (fromState, toState, toParams, options) {
                ...
                var info = avalon.router.urlFormate(toState.url, toParams, toParams.query), me = this, options = options || {},
                    ...
                    state = toChain[i], _local = _root._local, toLocals = [];
                if (!reload) {
    ...
    function broadCastBeforeUnload(exitChain, enterChain) {
        ...
        for (var i in lastLocal) {
            ...
            if ('$onBeforeUnload' in lastLocal[i].$ctrl) {
                if (lastLocal[i].$ctrl.$onBeforeUnload(exitChain[0], enterChain[0]) === false)
                    return false;
            }
        }
    }
    ...
    avalon.bindingHandlers.view = function (data, vmodels) {
        ...
        function update(firsttime, currentState, changeType) {
            ...
            if (_local && _local.$ctrl && _local.$ctrl.$onRendered)
                _local.$ctrl.$onRendered.apply(_element, [_local]);
        }
        update('firsttime');
