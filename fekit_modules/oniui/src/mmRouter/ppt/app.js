...
require("mmRouter/ppt/model/ppt");
require("mmRequest/mmRequest");
require("loading/avalon.loading");
require("mmRouter/ppt/highlight.pack");
require("mmRouter/ppt/markdown.min");
...
function () {
    avalon.state('ppt', {
        url: '/:pageNumber',
        controller: 'ppt',
        views: { '': { templateUrl: 'ppt/views/tpl.html' } },
        onEnter: function (pageNumber, resolve) {
            if (pageNumber === '')
                return avalon.router.redirect('/1');
            var ppt = avalon.vmodels.ppt;
            if (ppt._curentPage != pageNumber)
                ppt._curentPage = pageNumber;
        ...
        onLoad: function () {
            ...
            if (document.querySelectorAll) {
                avalon.each(document.querySelectorAll('.oni-mmRouter-enter'), function (i, node) {
                    ...
                    if ($node.hasClass('oni-mmRouter-leave'))
                        return;
