...
function () {
    ...
    */
    avalon.router.go = function (toName, params, options) {
        var from = mmState.currentState, to = StateModel.is(toName) ? toName : getStateByName(toName), replaceQuery = options && options.replaceQuery, params = params || {};
    ...
    var mmState = window.mmState = {
            ...
            popOne: function (chain, params, callback) {
                ...
                cur.done = function (success) {
                    ...
                    if (success !== false) {
                        if (me.activeState)
                            return me.popOne(chain, params, callback);
                    }
            ...
            transitionTo: function (fromState, toState, toParams, options) {
                ...
                var info = avalon.router.urlFormate(toState.url, toParams, toParams.query), me = this, options = options || {},
                    ...
                    reload = options.reload || this.isParamsChanged(), over, fromChain = fromState && fromState.chain || [], toChain = toState.chain, i = 0, state = toChain[i], _local = _root._local, toLocals = [];
                if (!reload) {
    ...
    avalon.state = function (stateName, opts) {
        ...
        avalon.router.get(state.url, function (params, _local) {
            ...
            avalon.each(state.views, function (name, view) {
                var promise = fromPromise(view, me.params), warnings = 'warning: ' + stateName + '\u72B6\u6001\u5BF9\u8C61\u7684\u3010' + name + '\u3011\u89C6\u56FE\u5BF9\u8C61';
    ...
    */
    ...
    StateModel.prototype = {
        formate: function (options) {
            ...
            var stateName = this.stateName, me = this, chain = stateName.split('.'), len = chain.length - 1;
            this.chain = [];
