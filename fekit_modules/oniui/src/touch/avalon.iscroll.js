var NScroll = require("./nscroll.js");
...
function () {
    ...
    avalon.bindingHandlers.iscroll = function (data, vmodels) {
        var element = data.element,
            // 绑定的 dom 节点
            args = data.value.match(/[^, ]+/g),
            // 分析参数，用逗号分割，第一个为配置所对应的参数key（后面的参数以后拓展）
            vm = vmodels[0],
            // 获取 VM
            options = avalon.mix({}, DEFAULT_OPT, vm.iscroll, element.dataset, args && args[1] ? vm[args[1]] : null),
            // merge 配置
            id = options.id || args && args[0] !== '$' && args[0] || 'iscroll' + setTimeout('1'),
            // jshint ignore:line
        ...
        if (eachAttr || repeatAttr) {
            ...
            vm.$watch(name, function () {
                var arr = vm[name], newArr = vm[realName], removeIndex = [], i;
                // 判断是否是第一次加载
                if (scroll) {
                    // 取消监听
                    ...
                    newArr.pushArray(JSON.parse(JSON.stringify(vm.$model[name].slice(0, options.showLines))));
                    // 初始化僵尸数组的数据
                    items = element.children[0].children;
