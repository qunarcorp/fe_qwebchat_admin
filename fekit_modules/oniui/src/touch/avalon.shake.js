...
function () {
    if ('ondevicemotion' in window) {
        ...
        function outLimit(x, y, z) {
            var ax = Math.abs(x), ay = Math.abs(y), az = Math.abs(z), max = Math.max(ax, ay, az);
            if (max < threshold)
                return false;
            if (max === ax)
                return 'x';
        ...
        avalon.eventHooks.shake = {
            ...
            deel: function (elem, fn) {
                return function (e) {
                    var nowXYZ = e.accelerationIncludingGravity,
                        // 重力加速度
                        now,
                        // 当前时间
                        pastTime,
                        // 距离上次devicemotion间隔时间
                        axle,
                        // 最大增量轴
                        direction;
                    // 方向
                    nowXYZ.z = nowXYZ.z - 9.8;
                    // 忽略重力加速度的Z
                    ...
                    if (axle) {
                        ...
                        if (pastTime > 1000) {
                            // 单向
                            if (lastXYZ[axle]) {
                                direction = (lastXYZ[axle] < 0 ? '-' : '+') + axle    // 双向
;
                            } else {
                                direction = axle;
                            }
