...
function () {
    ...
    function NScroll(el, opt) {
        var scroll = null, cur = {
                column: 0,
                row: 0
            }, options = _.extend({}, DEFAULT_OPT, opt), swap = el && el.children[0], itemList = [], topScroll = options.bindScrolls.x, leftScroll = options.bindScrolls.y, i, j, node;
        ...
        function computeItems(scrollX, scrollY, directionX, directionY) {
            var curColumn = 0, lastColumn = options.column.num - 1, curRow = 0, lastRow = options.row.num - 1;
            ...
            itemList.forEach(function (item) {
                var change = false, toColumn = item.column, toRow = item.row;
        ...
        function computeX(x) {
            var dis = Math.abs(x), num = parseInt(dis / options.column.width), nextX = -(dis % options.column.width > options.column.width / 2 ? num + 1 : num) * options.column.width;
            return {
                move: nextX !== x,
                x: nextX
            };
        ...
        function computeY(y) {
            var dis = Math.abs(y), num = parseInt(dis / options.row.height), nextY = -(dis % options.row.height > options.row.height / 2 ? num + 1 : num) * options.row.height;
            return {
                move: nextY !== y,
                y: nextY
            };
        ...
        function fixPosition() {
            var ret1 = computeX(scroll.x), ret2 = computeY(scroll.y);
