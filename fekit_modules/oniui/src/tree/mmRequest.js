...
module.exports = (
function () {
    var global = window, DOC = global.document, r20 = /%20/g, rCRLF = /\r?\n/g, encode = encodeURIComponent, decode = decodeURIComponent, rheaders = /^(.*?):[ \t]*([^\r\n]*)\r?$/gm,
        // IE的换行符不包含 \r
        rlocalProtocol = /^(?:about|app|app-storage|.+-extension|file|res|widget):$/, rnoContent = /^(?:GET|HEAD)$/, rquery = /\?/, rurl = /^([\w.+-]+:)(?:\/\/(?:[^\/?#]*@|)([^\/?#:]*)(?::(\d+)|)|)/,
        ...
        curl = DOC.URL, segments = rurl.exec(curl.toLowerCase()) || [], isLocal = rlocalProtocol.test(segments[1]), head = DOC.head || DOC.getElementsByTagName('head')[0],
        //HEAD元素
    ...
    avalon.ajax = function (opts, promise) {
        ...
        'success error'.replace(avalon.rword, function (name) {
            //绑定回调
            delete opts[name];
        });
    ...
    function isValidParamValue(val) {
        var t = typeof val;
        // 值只能为 null, undefined, number, string, boolean
            ...
            script: {
                ...
                respond: function (event, forceAbort) {
                    ...
                    if (forceAbort || execute) {
                        ...
                        if (!forceAbort) {
                            var args = typeof avalon[this.jsonpCallback] === 'function' ? [
                                    500,
                                    'error'
                                ] : [
                                    200,
